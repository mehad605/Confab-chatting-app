      ...
      if (googleUser != null) {
        ...
        if (firebaseUser != null) {
          ...
          if (email != null) {
            ...
            bool userExists = await _db.checkIfUserExists(email);
            if (!userExists) {
              String? uid = await _auth.registerUserUsingEmailAndPassword(
                  email, 'temporary_password');
              String? imageURL = photoURL; // Use the retrieved photo URL
              await _db.createUser(uid!, email, name!, imageURL!);
              await _auth.loginUsingEmailAndPassword(email, _password!);
            }
